...
def work(device="cpu", dsname="Celegans"):
    ...
    def selparam(trial):
        ...
        layer2 = trial.suggest_int("l2", 1, 3)
        hidden_dim_1 = trial.suggest_categorical("h1", [24, 32, 64])
        hidden_dim_2 = trial.suggest_categorical("h2", [24, 32, 64])
        ...
        return valparam(hidden_dim_1, hidden_dim_2, layer1, layer2, dp1, 0, dp3, lr)
    ...
    def valparam(hidden_dim_1, hidden_dim_2, layer1, layer2, dp1, dp2, dp3, lr):
        if bg.x.shape[1] > 0:
            ...
            mod = Net(bg.max_x, hidden_dim_1, hidden_dim_2, layer1, layer2, dp1, dp2,
                      dp3, fast_bsmm=args.f).to(device)
...
def testparam(device="cpu",
    ...
    def valparam(hidden_dim_1wl, hidden_dim_2wl, layer1, layer2, dp1, dp2, dp3, lr):
        if len(bg.x.shape) > 1:
            print("use_feat")
            mod = Net(bg.max_x,
                      hidden_dim_1wl,
            ...
            mod = Net(bg.max_x, hidden_dim_1wl, hidden_dim_2wl, layer1, layer2, dp1, dp2,
                      dp3, fast_bsmm=args.f).to(device)
        opt = Adam(mod.parameters(), lr=lr)

...
def train_routine(mod, opt, trn_ds, val_ds, tst_ds, verbose=False):
    ...
    for i in range(200):
        ...
        if early_stop > 200:
            break
    vprint(f"end test {best_tst:.4f} time {t2-t1:.4f} s")
...
def work(device="cpu", dsname="Celegans"):
    ...
    def selparam(trial: optuna.Trial):
        ...
        if random.random() < 0.1:
            ...
            val_ds = Dataset(*bg.split(1))
            tst_ds = Dataset(*bg.split(2))
        alpha = trial.suggest_float("alpha", 0.05, 0.50, step=0.05)
        ...
        dp0 = trial.suggest_float("dp0", 0.0, 0.8, step=0.1)
        dp1 = trial.suggest_float("dp1", 0.0, 0.8, step=0.1)
        dp3 = trial.suggest_float("dp3", 0.0, 0.8, step=0.1)
        dp4 = trial.suggest_float("dp4", 0.0, 0.8, step=0.1)
        ...
        act4 = trial.suggest_categorical("act4", [True, False])
        return valparam(dim1=dim1,
                        dim2=dim2,
                        alpha=alpha,
                        layer1=layer1,
                        layer2=layer2,
    ...
    def valparam(lr, **kwargs):
        if bg.x.shape[1] > 0:
            mod = Net_wxy(bg.max_x,
                          fast_bsmm=args.f,
                          use_feat=True,
                          feat=bg.x,
                          **kwargs).to(device)
        else:
            mod = Net_wxy(bg.max_x, fast_bsmm=args.f, **kwargs).to(device)
...
def testparam(device="cpu",
    ...
    def valparam(lr, **kwargs):
        if len(bg.x.shape) > 1:
            ...
            mod = Net_wxy(bg.max_x, fast_bsmm=args.f).to(device)
        opt = Adam(mod.parameters(), lr=lr)

...
def train(num_clu, nnodes, datalist, negdatalist, mod: nn.Module, opt: Optimizer, batch_size: int):
    losss = []
    for i in range(num_clu):
...
def routine(num_clu: int, num_epoch: int, lr: float, batch_size: int, **kwargs):
    ...
    nei = revperm[data.edge_index].to(device)
    neibin = torch.searchsorted(cld.partptr.to(device), nei, right=True) - 1
    neibin[0][neibin[0]!=neibin[1]] = -1
    neibin = neibin[0].to(device)
    ...
    for key1 in nes:
        for key2 in nes[key1]:
            tb = torch.searchsorted(cld.partptr.to(device), nes[key1][key2], right=True) - 1
            tb[0, tb[0]!=tb[1]] = -1
            nbin[key1][key2] = tb[0]

    model = buildmodel(max_x = 1).cuda()
    opt =  Adam(model.parameters(), lr=lr)

    for i in range(num_epoch):
        loss = train(num_clu, nnodes, datalist, negdatalist, model, opt, batch_size)

...
from utils.tool import get_ei2, to_directed, reverse
...
for run in range(args.num_runs):
    ...
    ei1, adj, features, labels, masks, num_node, num_type, num_relation, constants, relations, types, pairs, hits_true, r_hits_candidates = encoding_test(run, train_dataset, test_dataset)

    ei1 = torch.cat([ei1, ei1[[1,0]]])
    ei2 = get_ei2(num_node, ei1, ei1)
    ...
    edge2, edge2_r, features, model = edge2.to(device), edge2_r.to(device), features.to(device), model.to(device)
    ...
    output_test = model(features, edge2, edge2_r).cpu()

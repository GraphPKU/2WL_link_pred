...
def train(mod, opt, dataset, batch_size):
    ...
    num_batch = perm1.shape[0]// pos_batchsize 
    losss = []
    preds = []
    ys = []
...
def test(mod, dataset, test=False):
    ...
    y = dataset.y.reshape(-1, 2)[:, 0].to(torch.bool).cpu()
    if False:
        result = roc_auc_score(y, sig)
    else:
...
def train_routine(dsname, mod, opt, trn_ds, val_ds, tst_ds, epoch, verbose=False):
    ...
    for i in range(epoch):
        t0 = time.time()
        loss, trn_score = train(mod, opt, trn_ds, batch_size)
        t1 = time.time()
